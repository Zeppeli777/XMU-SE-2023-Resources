...
Vector3f RayColor(const Ray& ray, Scene& scene, int depth=0, bool test=false)
{
	...
	if (closest_hp.objIdx != -1)
	{
		...
		float light_distance = (lightpos - closest_hp.position).norm();
		for (int i = 0; i < scene.ObjectCount(); ++i)
		{
			HitInfo shadow_ht;
			if (scene.GetObjectPtr(i)->Hit(shadow_ray, shadow_ht))
			{
				if (shadow_ht.t > 0.001f && shadow_ht.t < light_distance)
				{
					isShadow = true;
					break;
				}
